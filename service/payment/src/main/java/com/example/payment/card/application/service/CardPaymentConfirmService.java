package com.example.payment.card.application.service;
...
import com.example.payment.orderpayment.application.usecase.OrderPaymentValidationUseCase;
import com.example.payment.common.common.exception.InvalidCardPaymentRequestException;
...
import com.example.payment.orderpayment.domain.service.OrderPaymentValidationItemData;
...
import com.example.payment.common.infrastructure.messaging.kafka.contract.CardConfirmResultStatus;
...
public class CardPaymentConfirmService implements CardPaymentConfirmUseCase {
    ...
    private void validateCommand(CardPaymentConfirmCommand command) {
        if (command == null) {
            throw new InvalidCardPaymentRequestException("移대뱶 寃곗젣 ?뱀씤 ?붿껌???꾩슂?⑸땲??");
        }
        if (command.buyerId() == null) {
            throw new InvalidCardPaymentRequestException("援щℓ??ID???꾩닔?낅땲??");
        }
        if (command.orderId() == null) {
            throw new InvalidCardPaymentRequestException("二쇰Ц ID???꾩닔?낅땲??");
        }
        if (command.paymentKey() == null || command.paymentKey().isBlank()) {
            throw new InvalidCardPaymentRequestException("paymentKey???꾩닔?낅땲??");
        }
        if (command.amount() == null || command.amount().compareTo(java.math.BigDecimal.ZERO) <= 0) {
            throw new InvalidCardPaymentRequestException("寃곗젣 湲덉븸? 0蹂대떎 而ㅼ빞 ?⑸땲??");
        }
    ...
    private OrderPaymentValidationData validateOrderPayment(CardPaymentConfirmCommand command) {
        ...
        if (validationData == null || validationData.orderItems() == null || validationData.orderItems().isEmpty()) {
            throw new InvalidCardPaymentRequestException("二쇰Ц 寃곗젣 寃利?寃곌낵??二쇰Ц ??ぉ???놁뒿?덈떎.");
        }
        ...
        if (orderItemsTotalAmount.compareTo(command.amount()) != 0) {
            throw new InvalidCardPaymentRequestException("二쇰Ц ??ぉ ?⑷퀎? ?붿껌 寃곗젣 湲덉븸???쇱튂?섏? ?딆뒿?덈떎.");
        }
    ...
    private static java.math.BigDecimal getOrderItemsTotalAmount(OrderPaymentValidationData validationData) {
        ...
        for (OrderPaymentValidationItemData orderItem : validationData.orderItems()) {
            if (orderItem == null) {
                throw new InvalidCardPaymentRequestException("二쇰Ц 寃곗젣 寃利?寃곌낵??鍮꾩뼱 ?덈뒗 二쇰Ц ??ぉ???ы븿?섏뼱 ?덉뒿?덈떎.");
            }
            if (orderItem.orderItemId() == null) {
                throw new InvalidCardPaymentRequestException("二쇰Ц 寃곗젣 寃利?寃곌낵??二쇰Ц ??ぉ ID媛 ?놁뒿?덈떎.");
            }
            if (orderItem.sellerId() == null) {
                throw new InvalidCardPaymentRequestException("二쇰Ц 寃곗젣 寃利?寃곌낵???먮ℓ??ID媛 ?놁뒿?덈떎.");
            }
            if (orderItem.lineAmount() == null || orderItem.lineAmount().compareTo(java.math.BigDecimal.ZERO) <= 0) {
                throw new InvalidCardPaymentRequestException("二쇰Ц 寃곗젣 寃利?寃곌낵??二쇰Ц ??ぉ 湲덉븸???щ컮瑜댁? ?딆뒿?덈떎.");
            }
    ...
    private void validateConfirmation(
        ...
        if (!Objects.equals(confirmation.orderId(), command.orderId().toString())) {
            throw new InvalidCardPaymentRequestException("?뱀씤??二쇰Ц ID媛 ?붿껌 二쇰Ц ID? ?쇱튂?섏? ?딆뒿?덈떎.");
        }
