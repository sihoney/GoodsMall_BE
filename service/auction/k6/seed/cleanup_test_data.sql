...
SET balance    = COALESCE(
                     ...
                     (SELECT wt.balance_after
                      ...
                      WHERE wt.wallet_id = w.wallet_id
                        AND wt.transaction_id NOT IN (SELECT tx_id FROM _test_wallet_tx_ids)
...
-- ============================================================
-- [STEP 10] 베이스라인 테스트 전용 wallet 삭제
-- seed_baseline_wallets.sql 로 생성된 row 자체를 제거
-- baseline 재실행 시 seed_baseline_wallets.sql 다시 실행 필요
-- ============================================================

SELECT '=== 베이스라인 wallet 삭제 전 확인 ===' AS section;
...
SELECT '=== 베이스라인 wallet 삭제 확인 (0이어야 정상) ===' AS section;
