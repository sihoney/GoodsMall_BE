...
public class MemberController {
    ...
    @PatchMapping("/me")
    @Operation(summary = "현재 회원 수정", description = "인증된 사용자의 프로필 정보를 수정합니다.")
    ...
    @Operation(summary = "현재 회원 비밀번호 변경", description = "인증된 사용자의 비밀번호를 변경합니다.")
    public ResponseEntity<ApiResponse<ChangePasswordResponse>> changeCurrentMemberPassword(
            @CurrentMember AuthenticatedMember authenticatedMember,
            @RequestBody ChangePasswordRequest request
    ) {
        return ResponseEntity.ok(ApiResponse.success(
                memberUsecase.changeCurrentMemberPassword(authenticatedMember.memberId(), request)
        ));
    }

    @GetMapping("/me/oauth-accounts")
    @Operation(summary = "현재 회원 연동 계정 조회", description = "현재 로그인한 사용자의 외부 계정 연동 목록을 조회합니다.")
    ...
    @Operation(summary = "현재 회원 연동 계정 해제", description = "현재 로그인한 사용자의 특정 외부 계정 연동을 해제합니다.")
