...
| 환불 정책 | 구매확정 전 `HELD escrow` 상태의 판매자 환불 처리 |
| 정산 연계 | 구매확정 시 정산 후보 생성 이벤트 발행 |
...
| Kafka | `Payment -> Settlement` 후보 생성 이벤트 | 모듈 분리 + 비동기 반영 | 정산 반영 트리거 |
