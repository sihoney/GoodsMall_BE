...
class SellerSettlementPayoutRequestedEventConsumerTest {
    ...
    void listen_validEvent_creditsWalletAndPublishesSuccess() {
        ...
        ArgumentCaptor<com.example.payment.infrastructure.messaging.kafka.contract.SellerSettlementPayoutResultMessage> captor =
                ArgumentCaptor.forClass(com.example.payment.infrastructure.messaging.kafka.contract.SellerSettlementPayoutResultMessage.class);
        verify(payoutResultEventPublisher).publish(captor.capture());
        org.assertj.core.api.Assertions.assertThat(captor.getValue().resultStatus())
                .isEqualTo(SellerSettlementPayoutResultStatus.SUCCESS);
        org.assertj.core.api.Assertions.assertThat(captor.getValue().settlementId()).isEqualTo(settlementId);
    }
    ...
    void listen_duplicateEvent_publishesSuccessWithoutWalletMutation() {
        ...
        WalletTransaction existingTransaction = WalletTransaction.create(
                ...
                9_000L,
                10_000L,
                com.example.payment.domain.enumtype.WalletTransactionType.SETTLEMENT,
                settlementId,
                "SETTLEMENT",
